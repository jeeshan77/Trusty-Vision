...
function PowerfulTools() {
  return (
    <section
      id="powerful-tools"
      className="w-full py-20 bg-gradient-to-b from-black via-gray-900 to-black"
    >
      <div className="container mx-auto text-center">
        <motion.h2
          ...
          viewport={{ once: true }}
          className="text-4xl md:text-5xl font-extrabold mb-14 -mt-8 text-white drop-shadow-lg"
        >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-10">
          {tools.map((tool, idx) => (
            <motion.div
              ...
              className="flex flex-col items-center justify-center p-8 bg-gray-950/80 rounded-2xl border border-gray-800 hover:border-indigo-500 cursor-pointer transition-all duration-300"
            >
              <div className="text-6xl mb-3 drop-shadow-md">{tool.icon}</div>
              <p className="text-lg font-semibold text-gray-200 group-hover:text-indigo-400 transition-colors duration-300">
                {tool.name}
              </p>
            </motion.div>
          ))}
      ...
      <div className="absolute top-6 left-6">
        <Link href="/" passHref>
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="px-6 py-3 rounded-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold text-base shadow-lg transition duration-300"
          >
            ⬅ Back to Home
          </motion.button>
        </Link>
