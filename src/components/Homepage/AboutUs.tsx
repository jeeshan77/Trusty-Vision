...
const Card = (props: CardProps) => {
  ...
  return (
    <div
      className={`relative flex min-h-full min-w-full flex-col md:flex-row items-center justify-between gap-8 py-5 md:mx-10`}
    >
      {imageOnLeft && (
        <Image
          alt='aboutus-image'
          className="h-[200px] w-auto rounded-2xl shadow-xl border-2 border-primary"
          src={image}
          width={300}
          height={300}
          draggable={false}
      ...
      <div className="flex-1 text-justify text-gray-300">
        {heading && <h3 className="mb-3 text-2xl md:text-3xl font-bold text-secondary">{heading}</h3>}
        <p>{text}</p>
      </div>
...
export default function HomepageAboutUs(props: AboutUsProps) {
  ...
  return (
    ...
    >
      ...
      {/* Section Heading */}
      <div className="relative hidden w-full items-center justify-center px-10 py-16  text-4xl md:text-5xl text-secondary md:flex">
        <div
          ref={bgTextRef}
          className="text-shadow absolute left-0 z-0 text-start text-[100px] md:text-[150px] uppercase tracking-wider text-background brightness-50 lg:left-[15%] lg:text-[180px]"
        >
          {`"About Us"`}
        </div>
        <div className="z-10 text-5xl md:text-6xl uppercase">What we do</div>
      </div>
